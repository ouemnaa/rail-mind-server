...
from agents.resolution_agent.resolver import (
    orchestrate,
...
class TestAgentExecution:
    ...
    @pytest.mark.skip(reason="Depends on real environment / network")
    def test_agent_result_has_timing(self, mock_run, sample_conflict, sample_context):
        """Test that agent results include execution timing."""
        mock_run.return_value = AgentResult(status='ok', execution_ms=100)
    ...
    @pytest.mark.skip(reason="Depends on real environment / computation")
    def test_agent_timeout_handling(self, mock_run, sample_conflict, sample_context):
        """Test that timeouts are handled properly."""
        mock_run.return_value = AgentResult(status='timeout', execution_ms=1)
        # Very short timeout
...
class TestLLMJudge:
    ...
    @patch('agents.resolution_agent.resolver.Groq')
    def test_judge_parses_response(self, mock_groq_class, sample_conflict, mock_judge_response):
        ...
        mock_completion.choices = [
            MagicMock(message=MagicMock(content=json.dumps(mock_judge_response)))
        ]
...
class TestOrchestrator:
    ...
    @patch('agents.resolution_agent.resolver.run_hybrid_rag_agent')
    @patch('agents.resolution_agent.resolver.run_mathematical_agent')
    @patch('agents.resolution_agent.resolver.call_llm_judge')
    ...
    @patch('agents.resolution_agent.resolver.run_hybrid_rag_agent')
    @patch('agents.resolution_agent.resolver.run_mathematical_agent')
    def test_orchestrate_preserves_raw_output(self, mock_math, mock_hybrid, sample_conflict):
        """Test that raw agent outputs are preserved unchanged."""
        mock_hybrid.return_value = AgentResult(status='ok', execution_ms=10, raw_result={'data': 1})
        mock_math.return_value = AgentResult(status='ok', execution_ms=10, raw_result={'data': 2})
